...
遇到 `request origin is not allowed` 或站点返回 HTML 登录/验证页时，扩展会在同源页面内重试一次：优先复用已打开的站点页面，没有页面时临时打开后台标签页，完成后关闭。Cookie 认证会先核对账号；如果登录已失效、账号已切换或仍需人机验证，请到站点完成登录/验证后再重试。
